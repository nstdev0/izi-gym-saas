...
export class UpgradeOrganizationPlanController implements ControllerExecutor<Record<string, string>, void> {
    ...
    async execute(input: Record<string, string>, organizationId?: string): Promise<void> {
        if (!input.plan) {
            throw new BadRequestError("Plan no especificado")
        }
        if (!organizationId) {
            throw new BadRequestError("Organization ID no especificado")
        }
        await this.useCase.execute(input.plan, organizationId);
    }
}