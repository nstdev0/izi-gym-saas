...
export function Header({ slug, mode = "organization" }: HeaderProps) {
  ...
  const displayName = isDev ? "Developer User" : user?.fullName || "Usuario";
  const displayEmail = isDev ? "dev@local.host" : user?.primaryEmailAddress?.emailAddress || "";
  const displayInitials = isDev ? "DV" : `${user?.firstName?.charAt(0) || ""}${user?.lastName?.charAt(0) || ""}`;

  return (
    <header className="sticky top-0 z-30 flex h-14 items-center gap-4 border-b border-border/40 bg-background/95 backdrop-blur-sm px-6 lg:h-[60px] transition-all">
